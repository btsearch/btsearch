...
import { CartesianGrid, Area as RechartsArea, AreaChart as RechartsAreaChart, XAxis as RechartsXAxis, YAxis as RechartsYAxis } from "recharts";
...
import { EvilBrush, type EvilBrushRange, useEvilBrush } from "@/components/evilcharts/ui/evil-brush";
...
function useAreaChart() {
  ...
  if (!context) {
    throw new Error("Area chart parts (<Area />, <XAxis />, …) must be used within <EvilAreaChart />");
  }
...
type EvilAreaChartBaseProps<TData extends Record<string, unknown>, TConfig extends Record<string, ChartConfig[string]>> = {
...
type EvilAreaChartProps<TData extends Record<string, unknown>, TConfig extends Record<string, ChartConfig[string]>> = EvilAreaChartBaseProps<
  TData,
  TConfig
>;
 ...
 */
export function EvilAreaChart<TData extends Record<string, unknown>, TConfig extends Record<string, ChartConfig[string]>>({
  config,
  data,
  children,
  className,
  ...
  const contextValue = useMemo<AreaChartContextValue>(
    ...
    [config, curveType, animationType, isStacked, isExpanded, isLoading, selectedDataKey, selectDataKey],
  );
  ...
  return (
    <AreaChartContext value={contextValue}>
      ...
      >
        ...
        >
          {children}
          {isLoading && <LoadingArea chartId={chartId} curveType={curveType} onShimmerExit={onShimmerExit} />}
        </RechartsAreaChart>
      </ChartContainer>
    </AreaChartContext>
  );
...
export function Area({
  ...
  const revealType: AreaAnimationType = shouldReduceMotion ? "none" : (animationType ?? defaultAnimation);
...
export function XAxis({ tickLine = false, axisLine = false, tickMargin = 8, minTickGap = 8, ...props }: XAxisProps) {
  ...
  return <RechartsXAxis tickLine={tickLine} axisLine={axisLine} tickMargin={tickMargin} minTickGap={minTickGap} {...props} />;
}

type YAxisProps = ComponentProps<typeof RechartsYAxis> & { locale?: string };
...
export function YAxis({
  ...
  const defaultFormatter = (v: unknown) => (typeof v === "number" ? v.toLocaleString(locale) : String(v));
  ...
  return (
    <RechartsYAxis
      ...
      width={width}
      tickFormatter={isExpanded ? axisValueToPercentFormatter : (tickFormatter ?? defaultFormatter)}
      {...props}
    />
  );
}
...
type TooltipProps = {
  ...
  defaultIndex?: number;
  cursor?: boolean;
  valueFormatter?: React.ComponentProps<typeof ChartTooltipContent>["valueFormatter"];
  locale?: string;
};

export function Tooltip({ variant, roundness, defaultIndex, cursor = true, valueFormatter, locale }: TooltipProps) {
  ...
  return (
    <ChartTooltip
      ...
      content={
        <ChartTooltipContent selected={selectedDataKey} roundness={roundness} variant={variant} valueFormatter={valueFormatter} locale={locale} />
      }
    />
  );
}
...
export function Legend({ variant, align = "right", verticalAlign = "top", isClickable = false }: LegendProps) {
  ...
  return (
    <ChartLegend
      ...
      content={<ChartLegendContent selected={selectedDataKey} onSelectChange={selectDataKey} isClickable={isClickable} variant={variant} />}
    />
  );
}
...
const getOpacity = (selectedDataKey: string | null, dataKey: string) => {
  if (selectedDataKey === null || selectedDataKey === dataKey) {
...
const resolveDots = (
  ...
  Children.forEach(children, (child) => {
    ...
    if (child.type === Dot) {
      ...
      dot = <ChartDot type={variant} dataKey={dataKey} chartId={id} fillOpacity={dotOpacity} maskId={maskId} />;
    }

    if (child.type === ActiveDot) {
      const { variant } = (child as ReactElement<DotProps>).props;
      activeDot = <ChartDot type={variant} dataKey={dataKey} chartId={id} fillOpacity={dotOpacity} />;
    }
  });
...
const RevealMask = ({ id, type }: { id: string; type: RevealAnimationType }) => {
  ...
  return (
    <mask id={`${id}-reveal-mask`} maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" x="0" y="0" width="100%" height="100%">
      {type === "edges-in" ? (
        <>
        ...
        <motion.rect {...reveal} x="0" y="0" width="100%" height="100%" fill="white" style={{ originX: SINGLE_REVEAL_ORIGIN[type] }} />
      )}
    </mask>
  );
};
 ...
 */
const ColorGradient = ({ id, dataKey, config, isExpanded }: StyleProps & { config: ChartConfig; isExpanded: boolean }) => {
  const colorsCount = getColorsCount(config[dataKey] ?? {});

  return (
    <linearGradient id={`${id}-colors-${dataKey}`} x1="0" y1="0" x2="1" y2="0" gradientUnits={isExpanded ? "userSpaceOnUse" : "objectBoundingBox"}>
      {colorsCount === 1 ? (
        <>
  ...
  return (
    <>
      ...
      <pattern id={`${id}-gradient`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-gradient-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-gradient-reverse`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-gradient-reverse-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-solid`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-solid-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-lines`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-lines-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-dotted`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-dotted-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-hatched`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-hatched-mask)`} />
      </pattern>
    </>
  );
};
  ...
  return (
    <>
      ...
      <pattern id={`${id}-unselected`} patternUnits="userSpaceOnUse" width="100%" height="100%">
        <rect width="100%" height="100%" fill={`url(#${id}-colors-${dataKey})`} mask={`url(#${id}-unselected-mask)`} />
      </pattern>
    </>
  );
};
 ...
 */
const LoadingArea = ({ chartId, curveType, onShimmerExit }: { chartId: string; curveType: CurveType; onShimmerExit: () => void }) => {
  return (
    <>
      <RechartsArea
        type={curveType}
