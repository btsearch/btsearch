...
import { type ChartConfig, ChartStyle, getColorsCount } from "@/components/evilcharts/ui/chart";
...
function EvilBrush({
  ...
  return (
    <div ref={containerRef} data-chart={skipStyle ? undefined : chartId} className={cn("group relative select-none", className)} style={{ height }}>
      ...
      {/* Left handle */}
      <BrushHandle side="left" position={leftPosition} label={showLabels ? getLabel(range.startIndex) : undefined} bind={bind("left")} />

      {/* Right handle */}
      <BrushHandle side="right" position={rightPosition} label={showLabels ? getLabel(range.endIndex) : undefined} bind={bind("right")} />
    </div>
  );
}
...
function MiniChart({
          ...
          <React.Fragment key={dataKey}>
            ...
            {variant === "area" && (
              <>
                ...
                <pattern id={`${chartId}-zm-fill-${dataKey}`} patternUnits="userSpaceOnUse" width="100%" height="100%">
                  <rect width="100%" height="100%" fill={`url(#${chartId}-zm-${dataKey})`} mask={`url(#${chartId}-zm-fill-mask-${dataKey})`} />
                </pattern>
              </>
            )}
          </React.Fragment>
  ...
  if (variant === "bar") {
    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 2, right: 0, bottom: 0, left: 0 }} barGap={2} barSize={14}>
          <defs>{defsContent}</defs>
          {keys.map((dk) => (
            <Bar
              key={dk}
