  ...
  return (
    <ChartContext.Provider value={{ config }}>
      ...
      >
        ...
        <RechartsPrimitive.ResponsiveContainer className="min-h-0 w-full flex-1 overflow-hidden" initialDimension={initialDimension}>
          {children}
...
export const generateEasedGradientStops = (steps = 17, minOpacity = 0.05, maxOpacity = 0.9) =>
...
export function useLoadingData(isLoading: boolean, count?: number, min?: number, max?: number) {
  ...
  const loadingData = React.useMemo(
    () => getLoadingData(count, min, max),
    // loadingDataKey toggle triggers re-computation when the shimmer exits
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [count, loadingDataKey],
  );

  return { loadingData, onShimmerExit };
}

export { ChartContainer, ChartStyle, axisValueToPercentFormatter, LoadingIndicator, getColorsCount };
