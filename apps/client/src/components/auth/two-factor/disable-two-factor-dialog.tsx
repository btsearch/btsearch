...
export function DisableTwoFactorDialog({ open, onOpenChange }: DisableTwoFactorDialogProps) {
  ...
  const { mutate: disableTwoFactor, isPending: isDisabling } = useDisableTwoFactor(authClient as TwoFactorAuthClient, {
    onSuccess: () => {
      ...
      <AlertDialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {requiresPassword ? twoFactorLocalization.passwordConfirmation : twoFactorLocalization.twoFactorDescription}
            </AlertDialogDescription>
          </AlertDialogHeader>

          {requiresPassword && (
            <Field>
              <FieldLabel htmlFor="disable-two-factor-password">{localization.auth.password}</FieldLabel>
