...
import { useSendTwoFactorOtp, useVerifyBackupCode, useVerifyTotp, useVerifyTwoFactorOtp } from "@better-auth-ui/react/plugins/two-factor";
...
export function TwoFactorChallenge({ className }: TwoFactorChallengeProps) {
  ...
  const [methods, setMethods] = useState<TwoFactorMethod[]>(() => (isHydrated ? readTwoFactorMethods() : ["totp", "otp"]));
  ...
  const { mutate: sendTwoFactorOtp, isPending: isSendingOtp } = useSendTwoFactorOtp(twoFactorClient, {
    onSuccess: () => {
  ...
  const { mutate: verifyTotp, isPending: isVerifyingTotp } = useVerifyTotp(twoFactorClient, { onError: () => setCode(""), onSuccess: onVerified });

  const { mutate: verifyTwoFactorOtp, isPending: isVerifyingOtp } = useVerifyTwoFactorOtp(twoFactorClient, {
    onError: () => setCode(""),
    onSuccess: onVerified,
  });

  const { mutate: verifyBackupCode, isPending: isVerifyingBackupCode } = useVerifyBackupCode(twoFactorClient, { onSuccess: onVerified });
  ...
  const verifyCode = (completedCode: string) => {
    if (isPending || needsOtpRequest || method === "backup" || completedCode.length !== codeLength) {
      return;
    }
  ...
  const alternatives: { key: ChallengeMethod; label: string }[] = [
    ...(method !== "totp" && methods.includes("totp")
      ...
      : []),
    ...(method !== "otp" && methods.includes("otp") ? [{ key: "otp" as const, label: twoFactorLocalization.useEmailedCode }] : []),
    ...(method !== "backup" && backupCodesEnabled ? [{ key: "backup" as const, label: twoFactorLocalization.useBackupCode }] : []),
  ];
          ...
          <FieldGroup>
            {method === "backup" ? (
              <Field>
                ...
                <Input id="backupCode" name="backupCode" autoComplete="one-time-code" autoFocus required disabled={isPending} />
              </Field>
            ) : (
              <OtpField
                autoFocus
                disabled={isPending || needsOtpRequest}
                label={method === "otp" ? twoFactorLocalization.emailedCode : twoFactorLocalization.authenticatorCode}
                length={codeLength}
                name="code"
                value={code}
                onChange={setCode}
            ...
            <div className="flex flex-col gap-3">
              {needsOtpRequest ? (
                ...
                <Button type="submit" disabled={isPending || (method !== "backup" && code.length !== codeLength)}>
                  {isPending && <Spinner />}
              ...
              {method === "otp" && otpRequested && (
                <Button type="button" variant="outline" disabled={isPending || isCoolingDown} onClick={() => sendTwoFactorOtp()}>
                  {isCoolingDown ? localization.auth.resendIn.replace("{{seconds}}", String(cooldown)) : localization.auth.resend}
                </Button>
              )}

              {alternatives.map((alternative) => (
                <Button type="button" variant="ghost" key={alternative.key} disabled={isPending} onClick={() => switchMethod(alternative.key)}>
                  {alternative.label}
                </Button>
              ))}
            </div>
