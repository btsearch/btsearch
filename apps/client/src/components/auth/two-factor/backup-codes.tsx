...
export function BackupCodes({ codes }: BackupCodesProps) {
  ...
  const getBackupCodesText = () => formatBackupCodesText(codes, twoFactorLocalization, window.location.origin);

  const copyCodes = () => copy(getBackupCodesText());

  return (
    <div className="flex flex-col gap-3">
      <p className="text-muted-foreground text-sm">{twoFactorLocalization.backupCodesDescription}</p>
