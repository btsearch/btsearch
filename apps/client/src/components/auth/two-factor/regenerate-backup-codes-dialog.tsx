      ...
      <AlertDialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {codes.length || !requiresPassword ? twoFactorLocalization.backupCodesDescription : twoFactorLocalization.passwordConfirmation}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          {codes.length ? (
            ...
            requiresPassword && (
              <Field>
                <FieldLabel htmlFor="regenerate-backup-codes-password">{localization.auth.password}</FieldLabel>
          ...
          <AlertDialogFooter>
            {!codes.length && <AlertDialogCancel disabled={isPending}>{localization.settings.cancel}</AlertDialogCancel>}

            <Button type="submit" disabled={isPending}>
              {isPending && <Spinner />}

              {codes.length ? twoFactorLocalization.done : twoFactorLocalization.regenerateBackupCodes}
            </Button>
          </AlertDialogFooter>
        </form>
      </AlertDialogContent>
