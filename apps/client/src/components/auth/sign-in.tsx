...
export function SignIn({ className, socialLayout, socialPosition = "bottom" }: SignInProps) {
  const { authClient, basePaths, emailAndPassword, localization, plugins, socialProviders, viewPaths, navigate, Link } = useAuth();
    ...
    <Card className={cn("w-full max-w-sm", className)}>
      ...
      <CardContent>
        <div className="flex flex-col gap-6">
          {socialPosition === "top" && (
            <>
              {socialProviders && socialProviders.length > 0 && <ProviderButtons socialLayout={socialLayout} view="signIn" />}
              ...
              <FieldGroup>
                <Field data-invalid={!!fieldErrors.email}>
                  ...
                  <Input
                    ...
                    onInvalid={(e) => {
                      ...
                      const msg = el.validity.valueMissing ? localization.auth.fieldRequired : localization.auth.invalidEmail;
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {plugins.flatMap((plugin) =>
                    (plugin.authButtons ?? []).map((AuthButton, index) => <AuthButton key={`${plugin.id}-${index.toString()}`} view="signIn" />),
                  )}
                </div>
              </FieldGroup>
            </form>
        ...
        <div className="flex flex-col gap-3 items-center w-full mt-4">
          {emailAndPassword?.enabled && emailAndPassword?.forgotPassword && (
            <Link href={`${basePaths.auth}/${viewPaths.auth.forgotPassword}`} className="self-center text-sm underline-offset-4 hover:underline">
