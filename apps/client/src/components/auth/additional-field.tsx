 ...
 */
function CopyButton({ getValue, isDisabled }: { getValue: () => string | undefined; isDisabled?: boolean }) {
  ...
  return (
    <InputGroupButton
      aria-label={copied ? localization.settings.copiedToClipboard : localization.settings.copyToClipboard}
      title={copied ? localization.settings.copiedToClipboard : localization.settings.copyToClipboard}
      onClick={handleCopy}
      disabled={isDisabled}
    >
...
export function AdditionalField({ name, field: configuredField, isPending, optionalLabel }: AdditionalFieldProps) {
  const field =
  ...
  if (field.render) {
    ...
    return <FieldRenderer name={name} field={field} isPending={isPending} optionalLabel={optionalLabel} />;
  }
  ...
  if (inputType === "hidden") {
    return (
      <input
        type="hidden"
        name={name}
        value={field.defaultValue == null ? "" : field.defaultValue instanceof Date ? field.defaultValue.toISOString() : String(field.defaultValue)}
      />
    );
  }
    ...
    return (
      <Field>
        ...
        <Input
          ...
          defaultValue={
            field.defaultValue == null ? undefined : typeof field.defaultValue === "number" ? field.defaultValue : String(field.defaultValue)
          }
          placeholder={field.placeholder}
          required={field.required}
          readOnly={field.readOnly}
      ...
      <Field>
        ...
        <InputGroup>
          ...
          {field.copyable ? (
            ...
            field.suffix != null && <InputGroupAddon align="inline-end">{field.suffix}</InputGroupAddon>
          )}
        </InputGroup>
  ...
  return (
    <Field>
      <div className="flex items-center justify-between gap-2">
        <FieldLabel htmlFor={name}>{field.label}</FieldLabel>
        <span className="text-sm text-muted-foreground tabular-nums">{formatter.format(value)}</span>
      </div>
      ...
      <div className="relative flex gap-2">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger
            ...
            aria-invalid={!!error}
            disabled={isPending || field.readOnly}
            className={cn(buttonVariants({ variant: "outline" }), "flex-1 justify-between font-normal", "data-[empty=true]:text-muted-foreground")}
          >
