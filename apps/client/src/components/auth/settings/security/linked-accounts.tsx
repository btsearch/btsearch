...
export function LinkedAccounts({ className }: LinkedAccountsProps) {
  const { authClient, multipleAccountsPerProvider, socialProviders } = useAuth();
  ...
  const availableProviders =
    multipleAccountsPerProvider === false ? socialProviders?.filter((provider) => !linkedProviderIds.has(getProviderId(provider))) : socialProviders;

  const allRows = [
    ...(linkedAccounts?.map((account) => ({
      key: account.id,
      account,
      provider: socialProviders?.find((provider) => getProviderId(provider) === account.providerId) ?? account.providerId,
    })) ?? []),
