...
export function LinkedAccount({ account, provider }: LinkedAccountProps) {
  ...
  const { data: accountInfo, isPending: isLoadingInfo } = useAccountInfo(authClient, { query: { accountId: account?.id ?? "" } });
  ...
  const displayName = accountData?.login || accountData?.username || accountInfo?.user?.email || accountInfo?.user?.name || account?.accountId;
    ...
    <Item>
      ...
      <ItemContent>
        ...
        {account && isLoadingInfo ? (
          ...
          <ItemDescription>{account ? displayName : localization.settings.linkProvider.replace("{{provider}}", providerName)}</ItemDescription>
        )}
      </ItemContent>
      <ItemActions>
        {account ? (
