...
export function getLocationTimeUs(location: NsgLocation): bigint | null {
...
export function getLocationTimeMs(location: NsgLocation): number | null {
...
export function prepareRouteLocations(locations: readonly NsgLocation[]): NsgLocation[] {
...
export function findClosestRouteLocation(locations: readonly NsgLocation[], timestampMs: number): NsgLocation | null {
  if (locations.length === 0 || !Number.isFinite(timestampMs)) return null;
  let low = 0;
  let high = locations.length;
  while (low < high) {
    const middle = Math.floor((low + high) / 2);
    if (getLocationTimeMs(locations[middle])! < timestampMs) low = middle + 1;
    else high = middle;
  }
  ...
  return timestampMs - getLocationTimeMs(earlier)! <= getLocationTimeMs(later)! - timestampMs ? earlier : later;
}
