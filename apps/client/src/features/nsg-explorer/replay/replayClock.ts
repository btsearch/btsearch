...
export function findReplayLocationIndex(locations: readonly NsgLocation[], playheadMs: number): number {
  if (!Number.isFinite(playheadMs)) return -1;
  let low = 0;
  let high = locations.length;
  while (low < high) {
    const middle = Math.floor((low + high) / 2);
    if (getLocationTimeMs(locations[middle])! <= playheadMs) low = middle + 1;
    else high = middle;
  }
  return low - 1;
}
