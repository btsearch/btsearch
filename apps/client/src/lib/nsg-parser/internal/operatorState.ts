...
export type OperatorContext = Pick<NsgCell, "slotId" | "subId" | "rat" | "elapsedUs" | "eventIndex">;

type Version = { elapsedUs: number; eventIndex: number };
type OperatorSnapshot = Version & { slotId: number; subId: number | null; operators: Map<string, ResolvedOperator> };
type StateChange = (state: OperatorSnapshot) => void;
...
function operatorsFromService(data: NsgJsonObject): Map<string, ResolvedOperator> {
  const operators = new Map<string, ResolvedOperator>();
  const ambiguous = new Set<string>();
  const root = numericOperator(data.operator);
  ...
  for (const network of data.networks) {
    ...
    const operator = numericOperator(network.registeredPLMN) ?? root;
    if (operator === null) {
...
function operatorForContext(state: OperatorSnapshot | undefined, context: OperatorContext): ResolvedOperator | null {
...
export class StreamingOperatorState {
  private readonly slots = new Map<number, OperatorSnapshot>();
  ...
  private setSlot(slotId: number, subId: number | null, event: NsgEvent, operators = new Map<string, ResolvedOperator>()): void {
    ...
    if (event.name === "subscriptionsChanged") {
      ...
      if (Array.isArray(data.subscriptions))
        for (const subscription of data.subscriptions) {
          ...
          next.set(
            contextKey(subscription.simSlotIndex, subscription.subscriptionId),
            cellOperator(subscription.mcc, subscription.mnc)?.plmn ?? null,
          );
        }
...
export class OperatorHistory {
  private readonly histories = new Map<string, OperatorSnapshot[]>();

  constructor(events: readonly NsgEvent[]) {
    const collector = new StreamingOperatorState((state) => {
    ...
    for (const event of ordered) collector.process(event);
  }

  get(context: OperatorContext): ResolvedOperator | null {
    if (!identifier(context.slotId) || !identifier(context.subId)) return null;
    const history = this.histories.get(contextKey(context.slotId, context.subId));
