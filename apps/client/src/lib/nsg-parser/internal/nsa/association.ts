...
function createDerivedCells(anchor: LteAnchor, observation: TimedNrMeasurement): NsgCell[] {
  ...
  for (let cellIndex = 0; cellIndex < observation.measurement.cells.length; cellIndex++) {
    ...
    const raw: NsgJsonObject = {
      ...
      diagStreamIndex: observation.streamIndex,
    ...
    const cell: NsgCell = {
      ...
      isDefaultSubscription: anchor.cell.isDefaultSubscription,
      mcc: anchor.cell.mcc,
      mnc: anchor.cell.mnc,
      operatorName: anchor.cell.operatorName,
      lac: null,
      rnc: null,
      cid: null,
      tac: null,
...
export function associateQualcommNsaMeasurements(
  ...
  for (const observation of observations) {
    const eligibleAnchors = resolveAnchors(observation.streamIndex, observation.elapsedUs);
    if (eligibleAnchors === null) continue;
